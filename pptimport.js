/* ============================================================
   ...
   x/y 指的是「文字方塊左上角」佔版面寬/高的比例,與下方判定式一致:
     編號:y < 10% 的純數字(1–4 位)
     照片:頁面最大的一張圖,面積須 ≥ 版面 8%(小 logo 不會被誤抓)
     姓名:x 30–55%、y ≤ 25%,單段、不含數字、8 字以內
     專業別:x ≥ 55%、y ≤ 28%(換行視為排版,會併回一行)
     服務項目:x 40–68%、y 20–50%,一段落=一項
     適合引薦對象:x 40–68%、y 50–80%
     宣傳標語:x > 68%、y > 60%
     左下角的引薦人姓名(y > 80% 且 6 字以內)會自動忽略
     落在以上區塊之外的文字 → 列入「未辨識」供人工確認,不會寫進名錄
   依賴:jszip.js
   ============================================================ */
