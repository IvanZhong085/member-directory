/* 名冊 CSV 的共用定義:欄位順序、「成員 → 一列」對應、跳脫規則。
   ...
   註:「照片/名片/商品照片數/最後更新」為唯讀欄(匯入不對應這幾欄,僅供核對)。 */
const CSV_SCHEMA = {
  HEADERS: ["編號","姓名","行業職稱","分組代號","分組名稱","服務項目","適合引薦對象","我有","我要","宣傳標語","所屬公司","主要營業項目","公司網站","照片","名片","商品照片數","最後更新","資料需確認","刪除"],
  memberRow(g, m){
    return [
      m.number || "", m.name || "", m.title || "", g.code || "", g.name || "",
      (m.services || []).join("|"), (m.targets || []).join("|"),
