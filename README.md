...
3. 打開名錄網站 → 點右下角**小齒輪** → 出現「管理員登入」畫面 → 按「**初次設定／重設**」，填入：
   - GitHub 帳號、repo 名稱、分支（`main`）、資料檔路徑（`data.js`）（通常已預填好）
   - 貼上剛剛的**權杖**，並設定一組**管理密碼**（日常登入用）→ 儲存。
4. 完成。權杖會用你的管理密碼**加密後**存在這台電腦的瀏覽器——之後日常只要輸入密碼，不必再碰權杖；沒有密碼的人打不開編輯模式、也發布不了。

> ⚠️ 權杖等於管理權限，**不要貼給別人、不要放進網頁檔案**。換電腦要重做這一步；權杖若外流，回 GitHub 把它 Revoke（撤銷）即可作廢。密碼忘記了就用「初次設定／重設」重貼權杖、設新密碼。
